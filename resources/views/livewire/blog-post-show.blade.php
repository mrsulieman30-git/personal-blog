@php
    ...
    $shareUrl = request()->fullUrlWithQuery(['lang' => app()->getLocale()]);
@endphp
@section('seo')
            ...
            <div class="container mx-auto px-4 md:px-8 max-w-5xl py-5 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    <button 
                        ...
                        @click="if(canShare) { navigator.share({ title: '{{ addslashes($post->title) }}', url: '{{ $shareUrl }}' }) } else { window.open('https://twitter.com/intent/tweet?url={{ urlencode($shareUrl) }}&text={{ urlencode($post->title) }}', '_blank') }"
